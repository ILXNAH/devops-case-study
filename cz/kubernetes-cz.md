...
1. [Kontejner, virtuální stroj (VM) a rozdíly mezi nimi](#kontejner-virtuální-stroj-vm-a-rozdíly-mezi-nimi)
   - [Kontejner](#kontejner)
   - [Virtuální stroj (VM)](#virtuální-stroj-vm)
...
15. [Instalace clusteru v on-prem prostředí](#instalace-clusteru-v-on-prem-prostředí)
   - [Přípravné kroky](#přípravné-kroky)
   - [Inicializace řídícího uzlu (Master Node)](#inicializace-řídícího-uzlu-master-node)
   - [Konfigurace kubectl](#konfigurace-kubectl)
   - [Konfigurace síťování (CNI pluginy)](#konfigurace-síťování-cni-pluginy)
   - [Připojení pracovních uzlů (Worker Nodes)](#připojení-pracovních-uzlů-worker-nodes)
   - [Instalace volitelných nástrojů](#instalace-volitelných-nástrojů)
   - [Nasazení aplikace](#nasazení-aplikace)
   - [Konfigurace clusteru](#konfigurace-clusteru)
16. [Upgrade clusteru v on-prem prostředí](#upgrade-clusteru-v-on-prem-prostředí)
   - [Ověření aktuální verze a kompatibility](#ověření-aktuální-verze-a-kompatibility)
   - [Záloha etcd a aplikačních dat](#záloha-etcd-a-aplikačních-dat)
   - [Upgrade řídících uzlů (Master Nodes)](#upgrade-řídících-uzlů-master-nodes)
   - [Upgrade pracovních uzlů (Worker Nodes)](#upgrade-pracovních-uzlů-worker-nodes)
   - [Ověření funkčnosti po upgradu](#ověření-funkčnosti-po-upgradu)
   - [Upgrade ostatních komponent a nástrojů](#upgrade-ostatních-komponent-a-nástrojů)
