...
5. **Připojení pracovních uzlů (Worker Nodes)**:
    - Na pracovních uzlech se připojte ke clusteru pomocí příkazu `kubeadm join`. <br> Příkaz `kubeadm join` se generuje po úspěšné inicializaci řídícího uzlu (`kubeadm init`).
